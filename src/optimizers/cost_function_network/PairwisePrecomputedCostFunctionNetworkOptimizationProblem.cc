	...
	Parent()
{
	...
	PairwisePrecomputedCostFunctionNetworkOptimizationProblem::protected_assign(src);
}
...
bool
PairwisePrecomputedCostFunctionNetworkOptimizationProblem::protected_empty() const {
	return single_node_penalties_.empty() &&
		pairwise_node_penalties_.empty() &&
		single_node_penalties_for_variable_nodes_.empty() &&
		interacting_variable_nodes_.empty() &&
		background_constant_offset_ == 0.0 &&
		one_choice_node_constant_offset_ == 0.0 &&
...
void
PairwisePrecomputedCostFunctionNetworkOptimizationProblem::protected_clear() {
...
void
PairwisePrecomputedCostFunctionNetworkOptimizationProblem::protected_make_independent() {
...
void
PairwisePrecomputedCostFunctionNetworkOptimizationProblem::protected_assign(
	masala::base::managers::engine::MasalaDataRepresentation const & src
) {
	PairwisePrecomputedCostFunctionNetworkOptimizationProblem const * src_ptr_cast( dynamic_cast< PairwisePrecomputedCostFunctionNetworkOptimizationProblem const * >( &src ) );
	CHECK_OR_THROW_FOR_CLASS( src_ptr_cast != nullptr, "protected_assign", "Could not assign an object of type " + src.class_name()
		+ " to an object of type PairwisePrecomputedCostFunctionNetworkOptimizationProblem."
	);
	single_node_penalties_ = src_ptr_cast->single_node_penalties_;
	pairwise_node_penalties_ = src_ptr_cast->pairwise_node_penalties_;
	single_node_penalties_for_variable_nodes_ = src_ptr_cast->single_node_penalties_for_variable_nodes_;
	interacting_variable_nodes_ = src_ptr_cast->interacting_variable_nodes_;
	background_constant_offset_.store( src_ptr_cast->background_constant_offset_.load() );
	one_choice_node_constant_offset_.store( src_ptr_cast->one_choice_node_constant_offset_.load() );
